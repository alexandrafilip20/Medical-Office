...
public class ReaderWriter
{
    ...
    public void readChild(DoctorOffice doctorOffice)
    {
        ...
        for(ArrayList<String> child : childList)
        {
            ...
            ChildPacient c = new ChildPacient(lastName, firstName, age, sex, diseases, new ArrayList<>(), parentName);
    ...
    public void readAdult(DoctorOffice doctorOffice)
    {
        ...
        for(ArrayList<String> adult : adultList)
        {
            ...
            AdultPacient a = new AdultPacient(lastName, firstName, age, sex, diseases, new ArrayList<>(), smoker);
    ...
    public void readDoctor(DoctorOffice doctorOffice)
    {
        ...
        for(ArrayList<String> doc : doctorList)
        {
            ...
            if(bodyPart.equals("plamani"))
            {
                ...
                Pulmonologist p = new Pulmonologist(lastName, firstName, age, qualification, bodyPart, isSurgeon ,new PriorityQueue<>(),bestTreatment);
                doctorOffice.getDoctors().add(p);
            }
            else if(bodyPart.equals("piele"))
            {
                int treatedCases = Integer.parseInt(doc.get(6));
                Dermatologist d = new Dermatologist(lastName, firstName, age, qualification, bodyPart, isSurgeon ,new PriorityQueue<>(),treatedCases);
                doctorOffice.getDoctors().add(d);
            }
            else  if(bodyPart.equals("inima"))
            {
                String recommendation = doc.get(6);
                Cardiologist c = new Cardiologist(lastName, firstName, age, qualification, bodyPart, isSurgeon ,new PriorityQueue<>(),recommendation);
    ...
    public void readAppointment(DoctorOffice doctorOffice) throws ParseException
    {
        ...
        for(ArrayList<String> appointment : appointmentList)
        {
            ...
            Appointment a = new Appointment(pacientLastName, pacientFirstName, doctorLastName, doctorFirstName, hour, date, reason, price);
